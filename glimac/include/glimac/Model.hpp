...
public:
    ...
    Model(string const &path, float scale, bool gamma = false) : gammaCorrection(gamma), scaleFactor(scale)
    {
        loadModel(path);
    }
    ...
    Mesh processMesh(aiMesh *mesh, const aiScene *scene)
    {
        ...
        for (unsigned int i = 0; i < mesh->mNumVertices; i++)
        {
            ...
            if (mesh->mTextureCoords[0]) // does the mesh contain texture coordinates?
            {
                ...
                vec.x = mesh->mTextureCoords[0][i].x * 10;
                vec.y = mesh->mTextureCoords[0][i].y * 10;
